...
almond_yield_model = function(#add climate_data_df
                              climate_data_df, 
  ...
  yield_result_df <- full_join(mean_tmin_c_month2, sum_precip_month1) %>% 
    ...
    select(year, almond_yield, precip_coeff, temp_coeff) # Becca initially just had "yield" here but Madi put in almond_yield because there is no column called yield
